...
void MiniKame_TurnR(MiniKame_t *robot, uint16_t steps, uint16_t T) {
    int16_t x_amp = 150;   // 15.0度
    int16_t z_amp = 150;   // 15.0度
    int16_t ap = -150;     // -15.0度
    int16_t hi = 230;      // 23.0度
    uint16_t period[] = { T, T, T, T, T, T, T, T, T };
    int16_t amplitude[] = { x_amp, x_amp, z_amp, z_amp, x_amp, x_amp, z_amp, z_amp, 0 };
    int16_t offset[] = { 900 + ap, 900 - ap, 900 - hi, 900 + hi, 900 - ap, 900 + ap, 900 + hi, 900 - hi, 450 };
    uint16_t phase[] = { 0, 180, 90, 90, 180, 0, 90, 90, 0 };
...
void MiniKame_TurnL(MiniKame_t *robot, uint16_t steps, uint16_t T) {
    ...
    uint16_t period[] = { T, T, T, T, T, T, T, T, T };
    int16_t amplitude[] = { x_amp, x_amp, z_amp, z_amp, x_amp, x_amp, z_amp, z_amp, 0 };
    int16_t offset[] = { 900 + ap, 900 - ap, 900 - hi, 900 + hi, 900 - ap, 900 + ap, 900 + hi, 900 - hi, 1350 };
    uint16_t phase[] = { 180, 0, 90, 90, 0, 180, 90, 90, 0 };
    MiniKame_Execute(robot, steps, period, amplitude, offset, phase);
}

void MiniKame_Dance(MiniKame_t *robot, uint16_t steps, uint16_t T) {
    int16_t x_amp = 0;
    int16_t z_amp = 400;   // 40.0度
    int16_t ap = -300;     // -30.0度
    int16_t hi = 200;      // 20.0度
    uint16_t period[] = { T, T, T, T, T, T, T, T, T };
    int16_t amplitude[] = { x_amp, x_amp, z_amp, z_amp, x_amp, x_amp, z_amp, z_amp, 0 };
    int16_t offset[] = { 900 + ap, 900 - ap, 900 - hi, 900 + hi, 900 - ap, 900 + ap, 900 + hi, 900 - hi, 900 };
    uint16_t phase[] = { 90, 90, 0, 270, 270, 270, 90, 180, 0 };
    MiniKame_Execute(robot, steps, period, amplitude, offset, phase);
}

void MiniKame_FrontBack(MiniKame_t *robot, uint16_t steps, uint16_t T) {
    int16_t x_amp = 300;   // 30.0度
    int16_t z_amp = 250;   // 25.0度
    int16_t ap = -200;     // -20.0度
    int16_t hi = 300;      // 30.0度
    uint16_t period[] = { T, T, T, T, T, T, T, T, T };
    int16_t amplitude[] = { x_amp, x_amp, z_amp, z_amp, x_amp, x_amp, z_amp, z_amp, 0 };
    int16_t offset[] = { 900 + ap, 900 - ap, 900 - hi, 900 + hi, 900 - ap, 900 + ap, 900 + hi, 900 - hi, 900 };
    uint16_t phase[] = { 0, 180, 270, 90, 0, 180, 90, 270, 0 };
    MiniKame_Execute(robot, steps, period, amplitude, offset, phase);
}

void MiniKame_Walk(MiniKame_t *robot, uint16_t steps, uint16_t T) {
    int16_t x_amp = 200;   // 20.0度
    int16_t z_amp = 350;   // 35.0度
    int16_t ap = 200;      // 20.0度
    int16_t hi = 150;      // 15.0度
    int16_t front_x = 120; // 12.0度
    uint16_t period[] = { T, T, T/2, T/2, T, T, T/2, T/2, T };
    int16_t amplitude[] = { x_amp, x_amp, z_amp, z_amp, x_amp, x_amp, z_amp, z_amp, 0 };
    int16_t offset[] = { 900 - ap - front_x, 900 + ap + front_x, 900 - hi + 100, 900 + hi - 100, 
                         900 + ap - front_x, 900 - ap + front_x, 900 + hi - 50, 900 - hi + 50, 0 };
    uint16_t phase[] = { 90, 90, 270, 90, 270, 270, 90, 270, 0 };
...
void MiniKame_Walkback(MiniKame_t *robot, uint16_t steps, uint16_t T) {
    ...
    int16_t offset[] = { 900 + ap + back_x, 900 - ap - back_x, 900 - hi, 900 + hi, 
                         900 - ap + back_x, 900 + ap - back_x, 900 + hi, 900 - hi, 0 };
    uint16_t phase[] = { 90, 90, 270, 90, 270, 270, 90, 270, 0 };
...
void MiniKame_Runback(MiniKame_t *robot, uint16_t steps, uint16_t T) {
    // 简化实现，与walkback类似但参数不同
    MiniKame_Walkback(robot, steps, T);
}

void MiniKame_MoonwalkL(MiniKame_t *robot, uint16_t steps, uint16_t T) {
    int16_t z_amp = 450;   // 45.0度
    uint16_t period[] = { T, T, T, T, T, T, T, T, T };
    int16_t amplitude[] = { 0, 0, z_amp, z_amp, 0, 0, z_amp, z_amp, 0 };
    int16_t offset[] = { 900, 900, 900, 900, 900, 900, 900, 900, 900 };
    uint16_t phase[] = { 0, 0, 0, 120, 0, 0, 180, 290, 0 };
    MiniKame_Execute(robot, steps, period, amplitude, offset, phase);
}

void MiniKame_UpDown(MiniKame_t *robot, uint16_t steps, uint16_t T) {
    int16_t z_amp = 1200;  // 120.0度
    int16_t hi = -300;     // -30.0度
    uint16_t period[] = { T, T, T, T, T, T, T, T, T };
    int16_t amplitude[] = { 0, 0, z_amp, z_amp, 0, 0, z_amp, z_amp, 0 };
    int16_t offset[] = { 900, 900, 900 - hi, 900 + hi, 900, 900, 900 + hi, 900 - hi, 900 };
    uint16_t phase[] = { 90, 270, 270, 90, 270, 90, 90, 270, 0 };
    MiniKame_Execute(robot, steps, period, amplitude, offset, phase);
}

void MiniKame_PushUp(MiniKame_t *robot, uint16_t steps, uint16_t T) {
    int16_t z_amp = 400;   // 40.0度
    int16_t x_amp = 650;   // 65.0度
    int16_t hi = 300;      // 30.0度
    uint16_t period[] = { T, T, T, T, T, T, T, T, T };
    int16_t amplitude[] = { 0, 0, z_amp, z_amp, 0, 0, 0, 0, 0 };
    int16_t offset[] = { 900, 900, 900 - hi, 900 + hi, 900 - x_amp, 900 + x_amp, 900 + hi, 900 - hi, 900 };
    uint16_t phase[] = { 0, 0, 0, 180, 0, 0, 0, 0, 0 };
    MiniKame_Execute(robot, steps, period, amplitude, offset, phase);
}
...
void MiniKame_Hello(MiniKame_t *robot) {
    uint16_t T = 800;
    uint16_t period[] = { T, T, T, T, T, T, T, T, T };
    int16_t amplitude[] = { 0, 400, 0, 400, 0, 0, 0, 0, 0 };  // 40.0度
    int16_t offset[] = { 900 + 150, 400, 900 - 650, 900, 900 + 100, 900 - 100, 900 + 200, 900 - 200, 900 };
    uint16_t phase[] = { 0, 0, 0, 90, 0, 0, 0, 0, 0 };
...
void MiniKame_Bye(MiniKame_t *robot) {
    uint16_t T = 800;
    uint16_t period[] = { T, T, T, T, T, T, T, T, T };
    int16_t amplitude[] = { 400, 400, 900, 900, 0, 0, 0, 20, 0 };  // 参数可能需要调整
    int16_t offset[] = { 900 + 500, 900 - 500, 900 - 150, 900 + 150, 900 + 200, 900 - 200, 900 + 100, 900 - 100, 900 };
    uint16_t phase[] = { 0, 0, 90, 90, 0, 0, 0, 0, 0 };
    MiniKame_Execute(robot, 4, period, amplitude, offset, phase);
}

void MiniKame_Afraid(MiniKame_t *robot) {
    int16_t afraidPositionLeft[] = { 900 + 150, 900 - 150, 900 - 650, 900 + 650, 900 + 200, 900 - 200, 900 + 100, 900 - 100, 900 - 50 };
    MiniKame_MoveServos(robot, 150, afraidPositionLeft);
    int16_t afraidPositionRight[] = { 900 + 150, 900 - 150, 900 - 650, 900 + 650, 900 + 200, 900 - 200, 900 + 100, 900 - 100, 900 + 50 };
...
void MiniKame_Confused(MiniKame_t *robot) {
    ...
    int16_t confusedPositionA[] = { 900 + ap, 900 - ap, 900 - hi + up, 900 + hi + down, 900 - ap, 900 + ap, 900 + hi - up, 900 - hi - down, 900 + 50 };
    MiniKame_MoveServos(robot, rand() % 500 + 200, confusedPositionA);
    vTaskDelay(pdMS_TO_TICKS(rand() % 600 + 800));
    int16_t confusedPositionB[] = { 900 + ap, 900 - ap, 900 - hi - down, 900 + hi - up, 900 - ap, 900 + ap, 900 + hi + down, 900 - hi + up, 900 - 50 };
...
void MiniKame_ShakeYes(MiniKame_t *robot, uint16_t steps, uint16_t T) {
    int16_t z_amp = 300;   // 30.0度
    int16_t hi = 300;      // 30.0度
    uint16_t period[] = { T, T, T, T, T, T, T, T, T };
    int16_t amplitude[] = { 0, 0, z_amp, z_amp, 0, 0, 0, 0, 0 };
    int16_t offset[] = { 900, 900, 900 - hi, 900 + hi, 900, 900, 900 + hi, 900 - hi, 900 };
    uint16_t phase[] = { 0, 0, 0, 0, 0, 0, 0, 0, 0 };
    MiniKame_Execute(robot, steps, period, amplitude, offset, phase);
}
...
void MiniKame_ShakeNo(MiniKame_t *robot) {
    int16_t ap = 200;
    int16_t hi = 350;
    int16_t shakeNoPositionLeft[] = { 900 + ap, 900 - ap, 900 - hi, 900 + hi, 900 - ap, 900 + ap, 900 + hi, 900 - hi, 900 - 450 };
    MiniKame_MoveServos(robot, 100, shakeNoPositionLeft);
    int16_t shakeNoPositionRight[] = { 900 + ap, 900 - ap, 900 - hi, 900 + hi, 900 - ap, 900 + ap, 900 + hi, 900 - hi, 900 + 450 };
...
void MiniKame_Headbang(MiniKame_t *robot, uint16_t steps, uint16_t T) {
    ...
    int16_t amplitude[] = { 0, 0, z_amp, z_amp, 0, 0, z_amp, z_amp, 0 };
    int16_t offset[] = { 900 + ap, 900 - ap, 900 - hi, 900 + hi, 900 - ap, 900 + ap, 900 + hi, 900 - hi, 900 };
    uint16_t phase[] = { 0, 0, 0, 180, 0, 0, 0, 180, 0 };
    MiniKame_Execute(robot, steps, period, amplitude, offset, phase);
}
...
void MiniKame_LookLeft(MiniKame_t *robot) {
    int16_t ap = -200;
    int16_t hi = 350;
    int16_t lookLeftPosition[] = { 900 + ap, 900 - ap, 900 - hi, 900 + hi, 900 - ap, 900 + ap, 900 + hi, 900 - hi, 900 + 450 };
...
void MiniKame_LookRight(MiniKame_t *robot) {
    int16_t ap = -200;
    int16_t hi = 350;
    int16_t lookRightPosition[] = { 900 + ap, 900 - ap, 900 - hi, 900 + hi, 900 - ap, 900 + ap, 900 + hi, 900 - hi, 900 - 450 };
...
void MiniKame_LookUp(MiniKame_t *robot) {
    int16_t ap = -200;
    int16_t hi = 350;
    int16_t lookUpPosition[] = { 900 + ap, 900 - ap, 900 - hi - 350, 900 + hi + 350, 900 - ap, 900 + ap, 900 + hi - 350, 900 - hi + 350, 900 };
...
void MiniKame_LookDown(MiniKame_t *robot) {
    int16_t ap = -200;
    int16_t hi = 350;
    int16_t lookDownPosition[] = { 900 + ap, 900 - ap, 900 - hi + 350, 900 + hi - 350, 900 - ap, 900 + ap, 900 + hi + 350, 900 - hi - 350, 900 };
...
void MiniKame_LookUpLeft(MiniKame_t *robot) {
    int16_t ap = -200;
    int16_t hi = 350;
    int16_t lookUpLeftPosition[] = { 900 + ap, 900 - ap, 900 - hi - 350, 900 + hi + 350, 900 - ap, 900 + ap, 900 + hi - 350, 900 - hi + 350, 900 - 450 };
...
void MiniKame_LookUpRight(MiniKame_t *robot) {
    int16_t ap = -200;
    int16_t hi = 350;
    int16_t lookUpRightPosition[] = { 900 + ap, 900 - ap, 900 - hi - 350, 900 + hi + 350, 900 - ap, 900 + ap, 900 + hi - 350, 900 - hi + 350, 900 + 450 };
...
void MiniKame_LookDownLeft(MiniKame_t *robot) {
    int16_t ap = -200;
    int16_t hi = 350;
    int16_t lookDownLeftPosition[] = { 900 + ap, 900 - ap, 900 - hi + 350, 900 + hi - 350, 900 - ap, 900 + ap, 900 + hi + 350, 900 - hi - 350, 900 - 450 };
...
void MiniKame_LookDownRight(MiniKame_t *robot) {
    int16_t ap = -200;
    int16_t hi = 350;
    int16_t lookDownRightPosition[] = { 900 + ap, 900 - ap, 900 - hi + 350, 900 + hi - 350, 900 - ap, 900 + ap, 900 + hi + 350, 900 - hi - 350, 900 + 450 };
...
void MiniKame_Center(MiniKame_t *robot) {
    int16_t ap = 200;
    int16_t hi = 350;
    int16_t centerPosition[] = { 900 + ap, 900 - ap, 900 - hi, 900 + hi, 900 - ap, 900 + ap, 900 + hi, 900 - hi, 900 };
...
void MiniKame_Rest(MiniKame_t *robot) {
    int16_t ap = 200;
    int16_t hi = -150;
    int16_t restPosition[] = { 900 + ap, 900 - ap, 900 - hi, 900 + hi, 900 - ap, 900 + ap, 900 + hi, 900 - hi, 900 };
    MiniKame_MoveServos(robot, 150, restPosition);
}

void MiniKame_Home(MiniKame_t *robot) {
    int16_t position[] = { 900, 900, 900, 900, 900, 900, 900, 900, 900 };
    for (int i = 0; i < NUM_SERVOS; i++) {
...
void MiniKame_Calibrate(MiniKame_t *robot) {
    int16_t hi = -150;
    int16_t position[] = { 900, 900, 900 - hi, 900 + hi, 900, 900, 900 + hi, 900 - hi, 900 };
    for (int i = 0; i < NUM_SERVOS; i++) {
        MiniKame_SetServo(robot, i, position[i]);
    }
}
...
void MiniKame_Pie(MiniKame_t *robot) {
    int16_t ap = 200;
    int16_t hi = 350;
    int16_t piePositionA[] = { 900 + ap, 900 - ap + 150, 900 - hi + 600, 900 + hi + 300, 900 - ap - 450, 900 + ap - 300, 900 + hi + 100, 900 - hi + 900, 900 - 50 };
    MiniKame_MoveServos(robot, 150, piePositionA);
    int16_t piePositionB[] = { 900 + ap, 900 - ap + 150, 900 - hi + 600, 900 + hi + 300, 900 - ap - 450, 900 + ap - 150, 900 + hi + 100, 900 - hi + 900, 900 - 50 };
...
void MiniKame_Imitate(MiniKame_t *robot, int16_t s0, int16_t s1, int16_t s2, int16_t s3, 
    ...
    int16_t position[] = { s0, s1, s2, s3, s4, s5, s6, s7, s8 };
    for (int i = 0; i < NUM_SERVOS; i++) {
        MiniKame_SetServo(robot, i, position[i]);
    }
}
